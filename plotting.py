...
def curve_individual(index, method, ax, runs, args):
  ...
  for run in runs:
    idx = 1
    for count in run.xs:
      if count <= args.xlim[-1]:
        idx += 1
    ax.plot(run.xs[:idx], run.ys[:idx], label=method, color=run.color, zorder=zorder)
...
def curve_std(index, method, ax, runs, args):
  ...
  if args.bins:
    ...
    for count in xs:
      if count <= args.xlim[-1]:
        idx += 1
    ax.fill_between(xs[:idx], ys[:idx] - std[:idx], ys[:idx] + std[:idx], **kw)
  ax.plot(xs[:idx], ys[:idx], label=method, color=color, zorder=10000 - 10 * index - 1)
  print(xs[idx-1], ys[idx-1], std[idx-1], method)
  return xs, ys
...
def binning(xs, ys, bins, reducer):
  binned_xs = np.arange(xs.min(), xs.max() + bins - 1, bins)
  # if last:
    # binned_xs = binned_xs + [xs[-1]]
  binned_ys = []
...
def legend(fig, mapping=None, **kwargs):
  ...
  # labels = [ 'DMVE(ours)', 'Dreamer', 'DQN(2e8 steps)', 'SimPLe(4e5 steps)', 'Random']
  # labels_1 = [ 'DMVE(ours)', 'Dreamer', 'DQN(2e8 steps)', 'SimPLe(4e5 steps)', 'Random']
  # new_entries = {}
  # for i in range(len(entries)):
  #   new_entries[labels_1[i]] = entries[labels[i]]
